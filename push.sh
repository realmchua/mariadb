...
while true; do
    if [[ -n "$result" ]]; then
        ...
        echo "$(docker images --format "{{.Repository}}:{{.Tag}}" --filter=reference=realmsg/mariadb)" >.repository
        cat .repository
        echo "" && echo "" && echo ""
        ImageFound=true
    else
    ...
    if [[ "$TagNPushLatest" == "true" ]]; then
        ...
        [yY])
            echo "Pushing to DockerHub as realmsg/mariadb:latest"
            docker tag $(sed '1q' .repository) realmsg/mariadb:latest
            docker push realmsg/mariadb:latest
            TagNPushStable=true
            echo "" && echo "" && echo ""
            ;;
        [nN])
    ...
    if [[ "$TagNPushStable" == "true" ]]; then
        ...
        [yY])
            echo "Pushing to DockerHub as realmsg/mariadb:stable"
            docker tag $(sed '1q' .repository) realmsg/mariadb:stable
            docker push realmsg/mariadb:stable
            echo "" && echo "" && echo ""
            break
            ;;
        [nN])
